$(document).ready(function () {
    ...
    function getWeather() { // on crée une fonction qui récupere la météo avec les instructions suivantes
        if (city == null) { // on teste si la variable city est nulle
            ...
            $.getJSON("http://api.openweathermap.org/data/2.5/weather?lang=fr&units=metric&APPID=" + myAPPID + "&q=" + city, function (result) { // on mets le résultat dans une variable result qui vaut le code JSON qu'on voit dans le navigateur
                var cityName = result.name; // le nom de la ville est directement accesible donc pas de souci
                var country = result.sys.country; // pays 
                ...
                var speedWind = result.wind.speed; // vitesse du vent
                ...
                cardSelector.append("<ul><li>" + cityName + " " + "(" +
                    country + ")" + "</li><li> <font size='70px'>" + temp + "</font> &deg;C</li></ul>" + "<ul><li>Humidite : " + humidity + "%" + "<li>vitesse du vent : " + speedWind + "kmh");
    ...
    function getWeatherFiveDays() { // on crée une fonction qui récupere les prévisions météor avec les instructions suivantes
        if (city == null) { // on teste si la variable city est nulle
            cardSelector.append("<p>Vous n'avez pas encore renseign&eacute; de ville.</p>"); // on affiche un message dans la card
        }else {
            $("#card *:not(div)").remove();
